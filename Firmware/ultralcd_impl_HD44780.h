...
void lcd_print(char c) { charset_mapper(c); }
  ...
  static void logo_lines(const char* const extra) {
    ...
    lcd.setCursor(indent, 0); lcd_printPGM(PSTR( " _     _" ));
    lcd.setCursor(indent, 1); lcd_printPGM(PSTR("|_)   |_)"));  
    lcd.setCursor(indent, 2); lcd_printPGM(PSTR( "|_)olo|_)erry.com" ));
  }
...
static void lcd_implementation_status_screen() {
  ...
  //
    ...
    bool blink = lcd_blink();
      ...
      lcd.setCursor(2, 1);
        ...
        lcd.print(ftostr52sp(current_position[X_AXIS]));

        lcd.print(' ');
        lcd.setCursor(11, 1);

        _draw_axis_label(Y_AXIS, PSTR(MSG_Y), blink);
        lcd.print(ftostr52sp(current_position[Y_AXIS]));
  ...
  //
    ...
    lcd.setCursor(LCD_WIDTH - len - 1, 0);
